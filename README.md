...
使用两个兼容 Chat Completions 的大模型 API，分别模拟 User 和 Character，结合用户画像、角色设定与随机主题逐轮生成对话。支持抽样计划、断点续跑、追加样本和训练数据导出。
...
当前已知问题：默认主题文件使用九字段结构，但 `load_catalog()` 仍强制校验旧的 `soft_tags`、`required_user_tags`、`excluded_character_tags` 数组。当前执行 `validate` 会报 `Invalid text list: soft_tags`，默认素材的 `plan` 和新批次 `generate` 也会被阻止。运行前需要统一加载器与主题数据结构；这些标签并未参与当前随机抽样。本次文档整理未修改生成代码。

先校验素材并运行离线测试：

```powershell
python dialogue_generate.py validate
python -m unittest test_dialogue_generate -v
```
...
`load_catalog()` 读取 `--schemes` 目录直属的所有 `topic_*.json`，将各文件的 `subtopics` 展开为一个主题列表。不会递归加载其他目录，也不会先抽大主题再抽子主题。

`make_jobs()` 枚举“角色 × 子主题 × User”全部组合，用 `random.Random(seed).sample(..., count)` 等概率、不放回抽取，再从每个选中子主题的 `seeds` 中随机选一条。

- 同次抽样不会重复完整三元组合；单个角色、用户或主题可以重复出现。
- 不按兴趣、职业、标签、服务名或意图名筛选和加权，不保证各主题覆盖次数相同。
- `id` 用于主题标识和唯一性校验；`group_id`、`group_name` 记录所属容器，不影响抽样权重。
- 固定种子在输入和顺序相同的条件下复现抽样，不保证远端模型回答一致。

`build_scene()` 按以下字段在本地组装场景，不使用画像扩写场景：

| 场景字段 | 来源 |
| --- | --- |
| `public.background` | 子主题 `name` |
| `public.trigger` | 抽中的 `seeds` 元素 |
| `public.boundary` | 子主题 `boundary` |
| `public.relationship` | 固定为“初次交流” |
| `public.facts` | 空列表 |
| `user_goal` | 程序中固定的交流目标说明 |
| `user_private`、`character_private` | 固定为“无” |

具体需求和情境细节由 User 在发言中展开。切换为原创合成素材：
...
流程为：抽样 → 本地场景 → User 发言 → Character 回复 → 本地结束判断；未结束则进入下一轮。批次和各轮 API 请求均串行执行。

User 接收自己的画像、公开场景、目标、角色公开姓名和公开历史；Character 接收自己的完整 profile、公开场景和公开历史。双方不会直接获得对方的完整画像。历史消息以当前发言者为视角转换：自己的发言标为 `assistant`，对方标为 `user`。
...
当前 User 提示词要求尽量至少进行 5 轮，但代码没有 5 轮的最低轮数约束。重复、Character 告别或拒答不会单独触发停止；不进行自动语义判定或最终质检。提示词位于主程序中的 `USER_PROMPT` 和 `CHARACTER_PROMPT`。
...
`--count` 表示本次新增数量，默认 5；新 ID 从现有最大编号之后开始。`--append` 允许切换素材或角色，不检查新组合是否与历史批次重复。`plan --append` 只追加计划。`--append` 和 `--resume` 不能同时使用；追加中断后应使用 `--resume`。
...
- `dialogue_00001.json`：完整轮次的公开对话、场景及目标，结束后增加 `completed_at`。首条 `system` 展示角色 profile，供人工评分参考，不作为此文件中的新增模型指令回传。
- `dialogue_00001.progress.log`：实际为 JSON，含 `events` 和 `state`，保存阶段、素材路径、调用用量及恢复信息。半轮中断时暂存待回复的 User 消息。
